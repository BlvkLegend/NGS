...
const COPY = {
  taxpayer: {
    eyebrow: "01 / What NGSC gives you",
  ...
  cruise: {
    eyebrow: "02 / wetin NGSC go give you",
    ...
    f3: { title: "Two modes, one record", body: "Taxpayer na for serious heads. Cruise na for the real ones." },
    cardShare: { title: "Your Nigerian Governance Scorecard", body: "Grade, score, full breakdown. Drop am, make dem see." },
    f5: { title: "Who dey lead", body: "Who dey rise. Who dey fall. Real time." },
    f6: { title: "Open your mouth", body: "Talk am after you score. Voice to text, nothing stored." },
    compare: { title: "Arrange them side by side", body: "Same ten questions. Let the numbers do the dragging." },
  },
};
  ...
  return (
    <section className="mx-auto max-w-[1400px] px-6 py-8 lg:px-10 lg:py-10">
      <div className="flex items-start justify-between gap-4">
        <div>
          <span className="ledger-index text-[12px] text-forest-500 dark:text-forest-300">{c.eyebrow}</span>
          <h2 className="mt-2 max-w-lg text-2xl font-bold leading-tight text-ink sm:text-3xl">
            {c.title}
          </h2>
        </div>
        {/* Leaderboard floating badge — matches design */}
        <a
          href={route("/leaders?view=rankings")}
          className="mt-1 flex shrink-0 items-center gap-1.5 rounded-full border border-cruise-500/60 bg-cruise-tint px-3 py-1.5 text-[12px] font-semibold text-cruise-700 transition-colors hover:bg-cruise-500 hover:text-paper dark:border-cruise-500/40 dark:bg-cruise-500/15 dark:text-cruise-300"
        >
          <span className="text-[14px] font-bold">⊕</span> Leaderboard
        </a>
      </div>
