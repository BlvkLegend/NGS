    ...
    <section className="mx-auto max-w-[1400px] px-6 py-10 lg:px-10 lg:py-12">
      ...
      >
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_1.6fr]">
          ...
          <TiltCard className="p-6" intensity={6}>
            {/* Two anonymous profile pills */}
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 rounded-full border border-line bg-paper px-2.5 py-1">
                <div className="h-4 w-4 rounded-full bg-forest-500/30" />
                <span className="text-[10px] font-semibold text-ink-muted">Official A</span>
              </div>
              <span className="font-mono text-[10px] font-bold text-ink-muted">VS</span>
              <div className="flex items-center gap-1.5 rounded-full border border-line bg-paper px-2.5 py-1">
                <div className="h-4 w-4 rounded-full bg-cruise-500/30" />
                <span className="text-[10px] font-semibold text-ink-muted">Official B</span>
              </div>
            </div>

            {/* Sector head-to-head bars */}
            <div className="mt-4 space-y-2">
              {[
                { sector: "Infrastructure", a: 81, b: 58 },
                { sector: "Healthcare",     a: 74, b: 69 },
                { sector: "Transparency",   a: 62, b: 80 },
                { sector: "Education",      a: 77, b: 55 },
              ].map((row) => (
                <div key={row.sector}>
                  <div className="mb-0.5 text-[9px] font-medium uppercase tracking-wide text-ink-muted">
                    {row.sector}
                  </div>
                  <div className="flex items-center gap-1">
                    {/* A bar — grows left from centre */}
                    <div className="flex flex-1 justify-end">
                      <div
                        className={`h-1.5 rounded-full ${row.a >= row.b ? "bg-signal-good" : "bg-line-strong"}`}
                        style={{ width: `${row.a}%` }}
                      />
                    </div>
                    {/* Centre line */}
                    <div className="w-px h-3 shrink-0 bg-line-strong" />
                    {/* B bar — grows right */}
                    <div className="flex flex-1">
                      <div
                        className={`h-1.5 rounded-full ${row.b > row.a ? "bg-signal-good" : "bg-line-strong"}`}
                        style={{ width: `${row.b}%` }}
                      />
                    </div>
                  </div>
            ...
            <div className="mt-3 flex items-center gap-1">
              <span className="flex-1 text-center font-mono text-[1.2rem] font-black text-signal-good">74</span>
              <div className="w-px h-4 bg-line-strong" />
              <span className="flex-1 text-center font-mono text-[1.2rem] font-black text-ink-muted">65</span>
            </div>

            <div className="mt-4 border-t border-line pt-4">
              <h3 className="font-semibold text-ink">{c.f4.title}</h3>
              <p className="mt-1 text-[12px] text-ink-muted">{c.f4.body}</p>
            </div>
          </TiltCard>
        </div>
