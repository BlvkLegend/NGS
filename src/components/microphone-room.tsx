...
export function MicrophoneRoom({ leaderName }: { leaderName?: string }) {
  ...
  return (
    <motion.section
      ...
      className="mt-14 overflow-hidden border-t border-line pt-10"
    >
      <span className="ledger-index text-[12px] text-forest-500">Microphone unlocked</span>
      ...
      <p className="mt-2 max-w-md text-[14px] leading-relaxed text-ink-muted">
        {mode === "cruise"
          ? `If you get more gist about ${leaderName ?? "the person wey you drag"}, talk am here. Press the mic, we go turn am to text. No audio dey saved.`
      ...
      {!supported ? (
        ...
        <div className="mt-6">
          <DepthButton onClick={toggleRecording} className={recording ? "!bg-[linear-gradient(180deg,#d97462_0%,#9c3b30_100%)]" : ""}>
            ...
            {recording ? (mode === "cruise" ? "Stop" : "Stop recording") : (mode === "cruise" ? "Press and talk" : "Start recording")}
          </DepthButton>

          <div className="mt-6 min-h-24 border border-line bg-paper-raised px-4 py-4">
            <p className="text-[14px] leading-relaxed text-ink">
              {transcript || (
                <span className="text-ink-muted">Your words will appear here as you speak.</span>
              )}
            </p>
          </div>
