...
// Directory: assembly interior, seated session. Rankings: assembly exterior, green dome.
const HERO_PHOTO: Record<Tab, string> = {
  directory: asset("/assembly-interior.webp"),
  rankings: "https://images.pexels.com/photos/33806113/pexels-photo-33806113/free-photo-of-nigerian-national-assembly-building-dome.jpeg?auto=compress&w=1600&h=700&fit=crop&dpr=1",
};
...
export function LeadersUnified() {
  ...
  // Sync tab state when the URL changes (e.g. navbar "Rankings" link while already on /leaders)
  const viewParam = searchParams.get("view") as Tab | null;
  const prevViewRef = useRef(viewParam);
  if (prevViewRef.current !== viewParam) {
    prevViewRef.current = viewParam;
    setTab(viewParam ?? "directory");
  }
  ...
  return (
    <div>
      {/* Photo band, swaps by tab: assembly interior for directory, exterior dome for rankings */}
      <div className="relative -mx-6 mb-6 h-40 overflow-hidden lg:-mx-10 sm:h-48 lg:h-56">
        <img
          key={tab}
          src={HERO_PHOTO[tab]}
          alt=""
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to right, var(--paper) 0%, color-mix(in srgb, var(--paper) 70%, transparent) 45%, transparent 80%)",
          }}
        />
        <div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-paper to-transparent" />
      </div>
