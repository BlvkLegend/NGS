...
const VARIANTS = [
  { id: "auto",     label: "Auto",    gradient: "",                                                                   swatch: "bg-gradient-to-br from-forest-500 to-forest-900" },
  { id: "forest",   label: "Forest",  gradient: "bg-[linear-gradient(145deg,#3fae74_0%,#0e5236_50%,#020e07_100%)]",  swatch: "bg-gradient-to-br from-[#3fae74] to-[#020e07]" },
  { id: "sunrise",  label: "Sunrise", gradient: "bg-[linear-gradient(145deg,#ffb765_0%,#d9720f_45%,#0e5236_100%)]",  swatch: "bg-gradient-to-br from-[#ffb765] to-[#0e5236]" },
  { id: "midnight", label: "Night",   gradient: "bg-[linear-gradient(145deg,#1a2a22_0%,#060d09_100%)]",              swatch: "bg-gradient-to-br from-[#1a2a22] to-[#060d09]" },
  { id: "naija",    label: "Naija",   gradient: "bg-[linear-gradient(145deg,#008751_0%,#004d30_50%,#000000_100%)]",  swatch: "bg-gradient-to-br from-[#008751] to-[#000000]" },
  { id: "fire",     label: "Fire",    gradient: "bg-[linear-gradient(145deg,#ff6b35_0%,#9c3b30_50%,#1a0a08_100%)]", swatch: "bg-gradient-to-br from-[#ff6b35] to-[#1a0a08]" },
] as const;

// Category bar without icons — score number + label + bar, all in paper/white
function CategoryBar({ label, score }: { label: string; score: number }) {
  return (
    <div className="flex items-center gap-2">
      <span className="w-[5.5rem] shrink-0 truncate text-[9px] font-medium uppercase tracking-wide text-paper/70">
        {label}
      </span>
      <div className="flex-1 h-1 rounded-full bg-paper/20 overflow-hidden">
        <div className="h-full rounded-full bg-paper/80" style={{ width: `${score}%` }} />
      </div>
...
export function ScorecardCanvas({ leader }: { leader: Leader }) {
  ...
  // Landmark background for the page — deterministic variant from score so each result feels slightly unique
  ...
  return (
    <div className="relative min-h-[calc(100vh-80px)] overflow-hidden">
      ...
      <img
        ...
        loading="eager"
      />
      {/* Dark vertical gradient — matches hero / quiz language */}
      <div
        aria-hidden
        className="absolute inset-0"
        style={{
          background: "linear-gradient(to bottom, rgba(0,0,0,0.75) 0%, rgba(0,0,0,0.40) 20%, rgba(0,0,0,0.55) 50%, rgba(0,0,0,0.88) 80%, rgba(0,0,0,0.97) 100%)",
        }}
      />
      ...
      <div className="relative mt-6">
        ...
        >
          {/* Adire-inspired corner geometry */}
          <div aria-hidden className="pointer-events-none absolute right-0 top-0 h-40 w-40 opacity-[0.08]">
            <svg viewBox="0 0 160 160" fill="none">
              {[0,1,2,3].map(i => (
                <circle key={i} cx="160" cy="0" r={40 + i * 28} fill="none" stroke="white" strokeWidth="1"/>
              ))}
              {[0,1,2,3,4,5].map(i => (
                <line key={i} x1="160" y1="0"
                  x2={160 - 140 * Math.cos(i * 30 * Math.PI/180)}
                  y2={140 * Math.sin(i * 30 * Math.PI/180)}
                  stroke="white" strokeWidth="0.5"/>
              ))}
            </svg>
          </div>
          <div aria-hidden className="pointer-events-none absolute -bottom-16 -left-16 h-56 w-56 rounded-full bg-paper/10 blur-2xl" />

          <div className="relative px-6 pt-7">
            <div className="flex items-center justify-between">
              <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.22em] text-paper">NGSC</p>
              <p className="font-mono text-[9px] uppercase tracking-[0.16em] text-paper/80">{leader.jurisdiction}</p>
            </div>
            <div className="mt-6 flex items-end justify-between">
              <div>
                ...
                {/* Role: always white, full opacity */}
                <p className="mt-0.5 text-[11px] font-medium text-paper">{leader.role}</p>
              </div>
              <div className="flex flex-col items-end">
                {/* Score number: drop-shadow ensures legibility on any gradient */}
                <p className="font-mono text-[5rem] font-black leading-none text-paper drop-shadow-lg">
                  {score}
                </p>
                <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-paper">/ 100</p>
              </div>
            </div>
          </div>

          <div className="relative px-6 pb-6">
            <div className="rounded-xl bg-paper/15 px-4 py-3 backdrop-blur-sm">
              {/* Rank label: always white, legible */}
              <p className="mb-2.5 text-[10px] font-bold uppercase tracking-[0.16em] text-paper">NGSC verdict</p>
            ...
            <div className="mt-4 flex items-center justify-between border-t border-paper/15 pt-3">
              <p className="text-[9px] uppercase tracking-[0.18em] text-paper/70">ngsc.africa</p>
              <p className="font-mono text-[9px] text-paper/70">#accountability</p>
            </div>
          </div>
        </motion.div>
