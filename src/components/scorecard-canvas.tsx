...
function CategoryBar({ label, score }: { label: string; score: number }) {
  return (
    <div className="flex items-center gap-2">
      <span className="w-[5.5rem] shrink-0 truncate text-[9px] font-medium uppercase tracking-wide text-paper/72">
        {label}
      </span>
      <div className="flex-1 h-[3px] rounded-full bg-paper/20 overflow-hidden">
        <div className="h-full rounded-full bg-paper/85" style={{ width: `${score}%` }} />
      </div>
      <span className="font-mono text-[9px] font-semibold text-paper/80 w-5 text-right">{score}</span>
    </div>
  );
}
...
function TaxpayerCard({
  ...
  const logoSrc = asset("/ngsc-logo.png");
  return (
    <div className="relative mx-auto flex aspect-[9/16] w-full max-w-[320px] flex-col justify-between overflow-hidden rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.4)]">
      ...
      <div className="relative px-5 pt-5">
        ...
        <div className="flex items-center justify-between">
          <p className="font-mono text-[7px] font-bold uppercase tracking-[0.22em] text-paper/80">
            Nigeria Governance Scorecard
          </p>
          {/* NGSC logo mark */}
          <img src={logoSrc} alt="NGSC" className="h-6 w-auto opacity-80" />
        </div>
        <p className="mt-0.5 font-mono text-[7px] uppercase tracking-[0.16em] text-paper/55">
          {leader.jurisdiction}
        </p>

        {/* Photo + name + grade — tight row, no large gap */}
        <div className="mt-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border-2 border-paper/40 bg-paper/12 text-paper">
              <UserRound size={18} strokeWidth={1.5} />
            </div>
            <div>
              <h2 className="text-[1.05rem] font-bold leading-tight text-paper drop-shadow-sm">
                {leader.name}
              </h2>
              <p className="text-[9px] font-medium text-paper/70">{leader.role}</p>
            </div>
          </div>

          {/* Grade stamp */}
          <div className="flex shrink-0 flex-col items-center rounded-xl border-2 border-paper/50 bg-paper/12 px-2.5 py-1.5 backdrop-blur-sm">
            <span className="font-mono text-[2.4rem] font-black leading-none text-paper drop-shadow">
              {grade}
            </span>
            <span className="font-mono text-[7px] text-paper/60">{score}/100</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative px-6 pb-6">
        ...
        <div className="rounded-xl border border-paper/20 bg-paper/12 px-4 py-3 backdrop-blur-md">
          <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-paper/60">NGSC Verdict</p>
          <p className="mt-1 text-[1rem] font-bold italic text-paper leading-tight">{rank}</p>
        </div>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-paper/20 pt-3">
          <p className="text-[8px] uppercase tracking-[0.2em] text-paper/55">ngsc.africa</p>
          <p className="font-mono text-[8px] text-paper/55">#accountability</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative mx-auto flex aspect-[9/16] w-full max-w-[320px] flex-col justify-between overflow-hidden rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.5)]">
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              <div className="flex items-center justify-between">
                <p className="font-mono text-[7px] font-bold uppercase tracking-[0.22em] text-paper/70">NGSC · Agbado Cruise</p>
                <img src={asset("/ngsc-logo.png")} alt="NGSC" className="h-5 w-auto opacity-75" />
              </div>
              <p className="font-mono text-[7px] uppercase tracking-[0.14em] text-paper/50">{leader.jurisdiction}</p>
              <div className="mt-2 flex items-center gap-2">
                <div className="flex h-10 w-10 items-center justify-center rounded-full border border-paper/30 bg-paper/12 text-paper">
                  <UserRound size={16} strokeWidth={1.5} />
                </div>
                <div>
                  <h2 className="text-[1.0rem] font-bold leading-tight text-paper">{leader.name}</h2>
                  <p className="text-[9px] text-paper/65">{leader.role}</p>
                </div>
              </div>
            </div>
            {/* Score + grade */}
            <div className="flex flex-col items-center">
              <p className="font-mono text-[3.5rem] font-black leading-none text-paper drop-shadow-lg">{score}</p>
              <p className="font-mono text-[8px] text-paper/55">/ 100</p>
              <span
                className="mt-1 rounded-lg px-2 py-0.5 font-mono text-[0.9rem] font-black text-paper"
      ...
      <div className="relative px-5 pb-5">
        ...
        >
          <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-paper/55">NGSC Verdict</p>
          <p className="mt-1 text-[1.0rem] font-bold italic text-paper leading-tight">{rank}</p>
          ...
          <div className="mt-3 flex items-center justify-between border-t border-paper/15 pt-2">
            <p className="text-[8px] uppercase tracking-[0.18em] text-paper/50">ngsc.africa</p>
            <p className="font-mono text-[8px] text-paper/50">#accountability</p>
          </div>
        </div>
      </div>
    </div>
...
export function ScorecardCanvas({ leader }: { leader: Leader }) {
  ...
  // Page background — landmark photo regardless of mode
  ...
  return (
    <div className="relative min-h-[calc(100vh-80px)] overflow-hidden">
      ...
      <div className="relative mx-auto max-w-lg px-6 py-14">
        ...
        <div className="relative mt-6">
          ...
          {!unlocked && (
            <div className="absolute inset-0 flex flex-col items-center justify-end gap-4 rounded-2xl bg-ink/45 pb-10 backdrop-blur-sm">
              ...
              <p className="max-w-56 text-center text-[13px] font-medium text-paper">
                {mode === "cruise"
                  ? "Share before the microphone open for you"
                  : "Share your card to unlock the voice feature"}
              </p>
              <div className="flex flex-wrap items-center justify-center gap-2 px-4">
                <button onClick={() => shareTo("native")} className="flex items-center gap-1.5 rounded-full bg-paper px-4 py-2 text-[13px] font-medium text-ink hover:bg-paper/90">
                  <Share2 size={13} /> Share
                </button>
                <button onClick={() => shareTo("whatsapp")} className="rounded-full border border-paper/50 px-4 py-2 text-[13px] font-medium text-paper hover:bg-paper/10">
                  WhatsApp
                </button>
                <button onClick={() => shareTo("x")} className="rounded-full border border-paper/50 px-4 py-2 text-[13px] font-medium text-paper hover:bg-paper/10">
                  X
                </button>
                <button onClick={copyLink} className="flex items-center gap-1.5 rounded-full border border-paper/50 px-4 py-2 text-[13px] font-medium text-paper hover:bg-paper/10">
        ...
        <div className="mt-5">
          <p className="mb-2 text-center text-[10px] font-medium uppercase tracking-wide text-paper/55">
            Customize your card
          </p>
        ...
        {unlocked && (
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            <button onClick={() => shareTo("native")} className="flex items-center gap-1.5 rounded-xl bg-forest-500 px-4 py-2.5 text-[13px] font-semibold text-paper transition-colors hover:bg-forest-700">
              <Share2 size={13} /> {mode === "cruise" ? "Share am again" : "Share again"}
            </button>
            <button onClick={copyLink} className="flex items-center gap-1.5 rounded-full border border-paper/40 px-4 py-2 text-[13px] font-medium text-paper transition-colors hover:bg-paper/10">
              <Copy size={13} /> {copied ? "Copied" : "Copy link"}
            </button>
          </div>
        )}
        ...
        <div className="mt-6 flex flex-col items-center gap-2">
          <Link href={`/card/${leader.slug}?s=${score}`} className="flex items-center gap-1.5 text-[13px] font-medium text-paper/80 hover:text-paper">
            View the full record <ArrowRight size={14} />
          </Link>
          {unlocked && nextLeader && (
            <Link href={`/evaluate/${nextLeader.slug}`} className="flex items-center gap-1.5 text-[13px] font-medium text-paper/55 hover:text-paper/80">
