...
export function NgscCardVisual({
  ...
  return (
    <div className="relative w-full max-w-[380px] overflow-visible rounded-[10px] border border-line-strong bg-paper-raised shadow-card">
      {/* Header — person avatar + name */}
      <div className="flex items-start gap-3 px-6 pt-5">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border-2 border-line-strong bg-forest-tint">
          <span className="font-mono text-[13px] font-bold text-forest-700">{leader.photoInitials}</span>
        </div>
        <div>
          <span className="ledger-index text-[10px] font-medium text-forest-500">NGSC</span>
          <h3 className="font-display text-xl font-semibold leading-tight text-ink">{leader.name}</h3>
          <p className="mt-0.5 text-[12px] text-ink-muted">{leader.role} · {leader.jurisdiction}</p>
        </div>
        <span className="ml-auto ledger-index text-[10px] text-ink-muted">NG · {leader.tookOffice}</span>
      </div>
