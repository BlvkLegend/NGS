...
function CompareViewInner() {
  ...
  const left  = leftSlug ? leaders.find((l) => l.slug === leftSlug) : null;;
  ...
  return (
    <div>
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border-2 border-forest-900 shadow-[0_6px_24px_-6px_rgba(0,0,0,0.22)]">
        <div className="flex h-24 bg-[#0a2e22] dark:bg-[#0a2e22]">
          {left ? <SideLandmark leader={left} wins={leftWins} mirror={false} /> : <EmptySlot label={mode === "cruise" ? "Pick first oga" : "Select first official"} />}
                ...
                <p className="text-[13px] text-ink-muted">
                  {left && right && left.score !== right.score ? (
                    <>
                      ...
                      <span className="font-mono font-bold text-signal-good">{left && right ? Math.abs(left.score - right.score) : 0} pts</span>
                    </>
                  ) : (
                    mode === "cruise"
        ...
        {(!left || !right) && (
          <div className="bg-paper px-6 py-8 text-center text-[13px] text-ink-muted">
            {!left
              ? (mode === "cruise" ? "Choose two ogas above to start the comparison." : "Select two officials above to begin the comparison.")
              : (mode === "cruise" ? "Choose a second oga up top to start the comparison." : "Select a second official above to begin the comparison.")}
          </div>
        )}
      </div>
    </div>
