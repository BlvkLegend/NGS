...
export function LeaderSelectCanvas() {
  ...
  return (
    <section className="mx-auto max-w-[1400px] px-6 py-16 lg:px-10">
      <span className="ledger-index text-[12px] text-forest-500">Choose your target</span>
      ...
      <div className="mt-10">
        {filtered.length === 0 ? (
          <EmptyState
            ...
            body="Try a different name or state."
          />
        ) : mode === "cruise" ? (
          <ul className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {filtered.map((leader) => (
              <li key={leader.slug}>
                ...
                >
                  <span className="flex h-16 w-16 items-center justify-center rounded-full bg-cruise-tint text-cruise-700">
