...
export function ResearchTable() {
  ...
  function exportCsv() {
    const header = ["Official", "Office", "Jurisdiction", "Party", "Score", "Grade", "Evaluations", "Trend"];
  ...
  return (
    <div>
      <div className="flex items-center justify-between border-b border-line pb-4">
        <p className="text-[13px] text-ink-muted">{rows.length} officials in the current register</p>
        <button
          onClick={exportCsv}
            ...
            {rows.map((r) => (
              <tr key={r.slug} className="border-b border-line hover:bg-forest-tint/40">
                <td className="py-3 pr-6">
                  <Link href={route(`/leaders/${r.slug}`)} className="text-[13px] font-medium text-ink hover:underline">
                    {r.name}
                  </Link>
                </td>
