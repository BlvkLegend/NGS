      ...
      <div className="relative mx-auto max-w-[1400px] overflow-hidden">
        <div className="relative h-48 w-full overflow-hidden sm:h-60">
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-6 lg:px-10">
            <span className="ledger-index text-[12px] text-forest-500">{mode === "cruise" ? "How e dey work" : "How it works"}</span>
...
{mode === "cruise" ? "Every drag on NGSC follow the same ten questions. Whether na you or your neighbour do am, the score dey comparable." : "Every evaluation on NGSC follows the same structure regardless of which official is scored or which mode was used. That consistency is what makes one governor's grade comparable to another's."}
        </p>
        ...
        <section className="mt-16">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-ink">{mode === "cruise" ? "Ten questions, ten areas" : "Ten questions, ten categories"}</h2>
          </div>
          <p className="mt-3 max-w-xl text-[14px] leading-relaxed text-ink-muted">
{mode === "cruise" ? "Same question, different vibe. Taxpayer mode dey formal. Cruise mode dey real Naija." : "One question per category, asked in both formal English (Taxpayer mode) and Nigerian Pidgin (Cruise mode). Same underlying question, different register."}
          </p>
        ...
        <section className="mt-12">
          <div className="flex items-start gap-4 rounded-2xl border border-forest-500/30 bg-forest-tint/40 p-6">
            ...
            <div>
              <h3 className="text-[15px] font-semibold text-ink">{mode === "cruise" ? "Blocking bad-faith drags" : "Guarding against noise"}</h3>
              <p className="mt-2 text-[13px] leading-relaxed text-ink-muted">
{mode === "cruise"
                  ? "No system immune to people wey wan game am. NGSC roadmap includes photo and clip evidence to back drags, plus pattern detection to catch coordinated fake entries."
                  : "No system built on public submissions is immune to bad-faith entries. NGSC’s roadmap includes evidence-backed evaluations where an evaluator can attach a photo or clip to support a claim, and pattern-based review to flag coordinated or duplicate submissions before they affect a public score."}
              </p>
            </div>
          </div>
        </section>

        <section className="mt-10 flex items-center justify-between gap-4 rounded-2xl bg-forest-900 px-6 py-5">
          <div>
            <p className="font-semibold text-white">{mode === "cruise" ? "You don know the gist." : "Now you know how it works."}</p>
            <p className="mt-0.5 text-[13px] text-white/60">{mode === "cruise" ? "Time to drag somebody. Go do am." : "Put the categories to use. Rate an official now."}</p>
          </div>
          <Link
            href="/start"
          ...
          >
            {mode === "cruise" ? "Go drag one" : "Rate a leader"}
          </Link>
        </section>
      </main>
      <SiteFooter />
