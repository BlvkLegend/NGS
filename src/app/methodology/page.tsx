...
export default function MethodologyPage() {
  ...
  const C = mode === "cruise" ? {
    ...
    h1:       "How your score become a grade.",
    intro:    "Every evaluation on NGSC follow the same structure, no matter which oga you dey grade or which mode you use. Na that consistency make one governor score meaningful to compare with another.",
    s1title:  "Ten questions, ten areas",
    s1body:   "One question per category — for proper English (Taxpayer mode) and for real Naija Pidgin (Cruise mode). Same question, different register.",
    s2title:  "Each question get A to F grade",
    s2body:   "No star rating, no sliding scale. Letter grades with clear definition, so every citizen dey use the same standard.",
    s3title:  "Answers average into score, score map to grade",
    s3body:   "The ten letter grades convert to number (A=5, B=4, C=3, D=2, F=1), then scale to 100. That number map to the final letter grade:",
    s4title:  "Evaluations dey anonymously bracketed",
    s4body:   "Results fit be read by segment without anybody identifying the individual wey submit. Three brackets dey collected at onboarding:",
    s5title:  "Final card go everywhere",
    s5body:   "After evaluation, NGSC generate one shareable card — grade, score, verdict. Lock behind share wall so citizens must scatter am before they fit talk more.",
  } : {
    eyebrow:  "How it works",
    h1:       "How a score becomes a grade.",
    intro:    "Every evaluation on NGSC follows the same structure regardless of which official is scored or which mode was used. That consistency is what makes one governor's grade comparable to another's.",
    s1title:  "Ten questions, ten categories",
    s1body:   "One question per category, asked in both formal English (Taxpayer mode) and Nigerian Pidgin (Cruise mode). Same underlying question, different register.",
    s2title:  "Each question answered A to F",
    s2body:   "Not star ratings or numbers: letter grades with clear definitions so every evaluator uses the same standard.",
    s3title:  "Answers average into a score, score maps to a grade",
    s3body:   "The ten letter grades convert to a numeric average (A=5, B=4, C=3, D=2, F=1), scaled to 100. That number maps to a final letter grade:",
    s4title:  "Evaluations are anonymously bracketed",
    s4body:   "Results can be read by segment without ever identifying an individual evaluator. Three brackets are captured at onboarding:",
    s5title:  "The final card goes everywhere",
    s5body:   "After evaluation, NGSC generates a shareable card — grade, score, verdict. Locked behind a share wall so the record becomes public before the voice feature unlocks.",
  };
      ...
      <div className="relative mx-auto max-w-[1400px] overflow-hidden">
        <div className="relative h-48 w-full overflow-hidden sm:h-60">
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-6 lg:px-10">
            ...
            <h1 className="mt-3 max-w-xl text-3xl font-bold leading-tight text-ink sm:text-4xl lg:text-5xl">{C.h1}</h1>
          </div>
        </div>
      </div>

      <main className="mx-auto max-w-4xl px-6 py-10 lg:px-10">
<p className="max-w-2xl text-[15px] leading-relaxed text-ink-muted">{C.intro}</p>
        ...
        <section className="mt-16">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-ink">{C.s1title}</h2>
          </div>
<p className="mt-3 max-w-xl text-[14px] leading-relaxed text-ink-muted">{C.s1body}</p>
        ...
        <section className="mt-16 border-t border-line pt-12">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-ink">{C.s2title}</h2>
          </div>
<p className="mt-3 max-w-xl text-[14px] leading-relaxed text-ink-muted">{C.s2body}</p>
        ...
        <section className="mt-16 border-t border-line pt-12">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-ink">{C.s3title}</h2>
          </div>
<p className="mt-3 max-w-xl text-[14px] leading-relaxed text-ink-muted">{C.s3body}</p>
        ...
        <section className="mt-16 border-t border-line pt-12">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-ink">{C.s4title}</h2>
          </div>
<p className="mt-3 max-w-xl text-[14px] leading-relaxed text-ink-muted">{C.s4body}</p>
