...
export const rankTitles: { min: number; taxpayer: string; cruise: string; taxpayerSub: string; cruiseSub: string }[] = [
  {
    min: 80,
    taxpayer: "Happy Citizen",
    cruise: "Certified Landlord of the People",
    taxpayerSub: "This evaluator is genuinely impressed with this leader's performance. A rare distinction in these parts.",
    cruiseSub: "This oga dey deliver! Your evaluator don give maximum respect. E be like say dem get this job.",
  },
  {
    min: 65,
    taxpayer: "Cautious Optimist",
    cruise: "Correct Guy (For Now)",
    taxpayerSub: "Progress is visible and measurable. Trust is being built, cautiously.",
    cruiseSub: "E try, and the receipts dey show. But e still get work to do before full marks.",
  },
  {
    min: 50,
    taxpayer: "The Selective Deliverer",
    cruise: "Half Light, Half Darkness",
    taxpayerSub: "Some mandates met, others quietly shelved. A mixed record that demands more scrutiny.",
    cruiseSub: "E do something sha. But half-half. The people wan see more than press release.",
  },
  {
    min: 35,
    taxpayer: "The Frustrated Taxpayer",
    cruise: "Certified Grid Sunsetter",
    taxpayerSub: "Promises made, evidence absent. This evaluator is running critically low on patience.",
    cruiseSub: "Na announcement full the air, results no dey ground. Your evaluator don tire.",
  },
  {
    min: 0,
    taxpayer: "The Accountability Deficit",
    cruise: "The Japa Activist",
    taxpayerSub: "A comprehensive failure across the record. This evaluator has lost all measurable confidence.",
    cruiseSub: "E don finish us. This evaluator don mentally pack bag dey wait for visa appointment.",
  },
];
...
export function getRankSubtext(score: number, mode: "taxpayer" | "cruise") {
  const band = rankTitles.find((r) => score >= r.min) ?? rankTitles[rankTitles.length - 1];
  return mode === "cruise" ? band.cruiseSub : band.taxpayerSub;
}
